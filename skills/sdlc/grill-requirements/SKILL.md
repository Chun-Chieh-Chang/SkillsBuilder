...
## 🔗 Addy Osmani 精華整合 (from interview-me)

### 假設+信心度模型
在提問前，先寫下你對用戶需求的最佳理解（一句話），附上誠實的信心度（0-100%）：
```
HYPOTHESIS: 你想要一個方式來在 Standup 回答「我們做得怎樣？」，而「儀表板」是你腦海中的慣用方案。
CONFIDENCE: ~30% — 缺少：為誰、「指標」在上下文中的含義、成功的定義
```

### 一次一問 + 附上猜測
```
Q: <一個聚焦的問題>
GUESS: <你對答案的假設，以及產生此假設的推理>
```
**為何不批量提問：**
- 批量問題鼓勵用戶略讀和表面回答
- 第三個問題通常取決於第一個答案
...
**為何附上猜測：**
- 用戶對錯誤猜測的反應速度比從零生成答案更快
- 它暴露了你的假設，這正是訪談要揭露的

### 「想要 vs 應該想要」偵測
最危險的回答是用戶說的像「最佳實踐」而非他們真正想要的。警覺：
- 模式匹配的最佳實踐語言（「我要可擴展的」、「乾淨架構」）但無具體內容
- 慣例導向的回答（「大多數 App 的做法」）
- 出現「我大概應該...」、「好的工程實踐說...」

> 破解問句：*「如果你不需要向任何人解釋，你真正想要什麼？」*
...
- Success:      <一行 — 如何知道成功>
- Constraint:   <一行 — 約束條件>
- Out of scope: <一行 — 明確不做什麼>
```
> **「Out of scope」不可省略。** 一半的目標偏差來自對「不做什麼」的隱性分歧。

### 95% 信心度停止條件
> *你能否預測用戶對你接下來三個問題的反應？*
- 能 → 你已建立共同理解，停止訪談並產出重述
- 不能 → 還沒完成，繼續提問
